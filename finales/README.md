...
- Svarer kast og nødutgang for en gruppe alltid «Noe ble endret samtidig» (en ødelagt myntkast-rad i databasen, bare mulig etter manuell redigering), slett raden og prøv igjen. Bytt ut lagnavnene med gruppens nøkkel (lagene i alfabetisk rekkefølge med `|` mellom): `npx wrangler d1 execute konfaction --remote --command "DELETE FROM meta WHERE key='tie:Delta|Echo'"`
