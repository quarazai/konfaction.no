...
def gate_day(now=None):
 now=now or datetime.now(ZoneInfo(CONFIG['timezone']))
 return now.date().isoformat()=='2026-10-09'
class Handler(BaseHTTPRequestHandler):
 ...
 def allowed(self):
  s=self.session()
  return not gate_day() or bool(s and (s.get('admin') or s.get('public')))
 ...
 def do_GET(self):
  ...
  if path=='/api/gate':
   s=self.session()
   if not gate_day() or (s and (s.get('public') or s.get('admin'))):return self.reply(200,{'required':False})
   if not s:
    token=secrets.token_urlsafe(32);s={'csrf':secrets.token_urlsafe(32),'expires':time.time()+86400,'public':False,'question':secrets.randbelow(len(GATE_QUESTIONS))}
    with LOCK:SESSIONS[token]=s
    return self.reply(200,{'required':True,'question':GATE_QUESTIONS[s['question']][0]},self.cookie(token,86400))
   return self.reply(200,{'required':True,'question':GATE_QUESTIONS[s.get('question',0)][0]})
  if path=='/api/session':
   s=self.session();return self.reply(200,dict(admin=bool(s and s.get('admin')),csrf=s['csrf'] if s else None,gateRequired=gate_day(),tournamentDay=datetime.now(ZoneInfo(CONFIG['timezone'])).date().isoformat()==CONFIG['date']))
  ...
  if path.startswith('/pixel/'):
   ...
   raw=p.read_bytes();self.send_response(200);self.headers_common();self.send_header('Content-Type',mimetypes.guess_type(p)[0] or 'application/octet-stream');self.send_header('Content-Length',str(len(raw)));self.end_headers();self.wfile.write(raw);return
 ...
 def do_POST(self):
  ...
  if self.path=='/api/login':
   ...
   token=secrets.token_urlsafe(32);s=dict(csrf=secrets.token_urlsafe(32),expires=time.time()+28800,admin=True,public=True)
   with LOCK:
  ...
  if self.path=='/api/gate':
   s=self.session()
   if not gate_day():return self.reply(200,{'ok':True})
   if not s:return self.reply(401,{'error':'Last siden på nytt og prøv igjen.'})
   answer=data.get('answer','')
   if not isinstance(answer,str) or normalize(answer) not in GATE_QUESTIONS[s.get('question',0)][1]:return self.reply(401,{'error':'Ikke helt. Se i bibelteksten og prøv igjen.'})
   s['public']=True
